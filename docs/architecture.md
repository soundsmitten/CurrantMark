...
WebKit scroll positions. Showing the split starts the second pane near the
active pane's current heading. Cross-document navigation updates the whole
document session and therefore both panes; a window never contains unrelated
documents.
...
through that history and remain hidden until navigation begins. Folder links
present a Markdown-filtered Open panel rooted at that folder. Anchor links
scroll to IDs generated by the Markdown processor. Other URLs are handed to
the system for now; in-app web navigation remains a later, separate
document-session capability.
